...
export const mercadolivre: Provider = {
  ...
  async sync(ctx) {
    ...
    const done = new Set<string>(); // pacotes já processados nesta rodada
    while (cur && Date.now() < ctx.deadline) {
      ...
      for (let i = cur.idx; i < list.length; i++) {
        ...
        const first = list[i];
        if (!out.sample) out.sample = first;
        // O Bling identifica a venda pelo pacote (pack_id) quando o comprador leva vários itens juntos.
        // Agrupamos os pedidos do pacote numa única linha, com o frete contado uma vez.
        const key = String(first.pack_id ?? first.id);
        if (done.has(key)) continue;
        done.add(key);
        let group: any[] = [first];
        if (first.pack_id) {
          const pack = await soft(get(ctx, `/packs/${first.pack_id}`));
          const ids: string[] = (pack?.orders ?? []).map((x: any) => String(x.id));
          if (ids.length > 1) {
            group = [];
            for (const oid of ids) {
              const o = list.find((x) => String(x.id) === oid) ?? await soft(get(ctx, `/orders/${oid}`));
              if (o) group.push(o);
            }
          }
        }
        group = group.filter((o) => o.status !== "cancelled" && o.status !== "invalid");
        if (!group.length) continue;
        const items = group.flatMap((o) => (o.order_items ?? []).map((it: any) => ({
        ...
        const saleFee = group.reduce((a, o) => a + (o.order_items ?? []).reduce((b: number, it: any) => b + num(it.sale_fee) * num(it.quantity || 1), 0), 0);
        let state: string | null = null, city: string | null = null, shipCost = 0;
        const shipId = group.find((o) => o.shipping?.id)?.shipping?.id;
        if (shipId) {
          const sh = await soft(get(ctx, `/shipments/${shipId}`, { "x-format-new": "true" }));
          ...
          const costs = await soft(get(ctx, `/shipments/${shipId}/costs`));
        ...
        out.marketOrders.push({
          id: key, platform: "Mercado Livre", date: day(o.date_closed ?? o.date_created)!,
          gross: round(group.reduce((a, x) => a + num(x.total_amount), 0)), fee: round(saleFee + shipCost), fee_source: "Mercado Livre",
